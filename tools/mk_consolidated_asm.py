...
_NOTE_PAIRS = [
 ...
 (0xF09F1C,'    BLDVTU   MOVE.L #1,A0       SIZE IS ALWAYS ONE PAGE'),
 (0xF09F1C,'    BLDVTU01 MOVE.L VCTUBGN,A4  RESTORE ADDRESS OF COMINT ROUTINE'),
 (0xF09F1C,'    BLDVTU02 CMP.L  (A2)+,A4    IS VECTOR POINTING AT COMINT ROUTINE?'),
 (0xF09F1C,'             BEQ.S  BLDVTU04    BRANCH IF YES - DO NOT SET \'USED\' FLAG'),
 (0xF09F1C,'             MOVE.B D2,(A0)     SET \'USED\' FLAG'),
 (0xF09F1C,'  So $1FA00 (slot $0C66) is the VTU -- VECTOR TABLE FOR USERS -- one byte'),
 (0xF09F1C,'  per exception vector.  The register described here as "a reference handler'),
 (0xF09F1C,'  in a4" is VCTUBGN, the address of the COMINT (common interrupt) routine,'),
 (0xF09F1C,'  and the $FF byte is a USED FLAG: set for every vector NOT pointing at the'),
 (0xF09F1C,'  common handler, i.e. every vector somebody has claimed.  Which also'),
 (0xF09F1C,'  explains why the table carries no eye-catcher -- BLDVTU writes none.'),
 (0xF09F1C,'  The later fill still holds: $F0226A overwrites each connected vector\'s'),
 (0xF09F1C,'  byte with the OWNING TASK NUMBER.  So the VTU\'s life is used-flags at'),
 (0xF09F1C,'  init, task numbers as vectors are connected.'),
 (0xF0011A,'THE REAL !VCT IS HERE, IN ROM -- linked into the kernel image, preceded by'),
 (0xF0011A,'  the longword $00F00186 (an address, not an opcode).  VECTINIT searches for'),
 (0xF0011A,'  it from ESTART within $200 bytes, stepping TWO bytes at a time, and'),
 (0xF0011A,'  BSR KILLERs if absent.  So this project\'s claim that !VCT "has kernel code'),
 (0xF0011A,'  but no tagged instance" is WRONG: the instance is in ROM, invisible to a'),
 (0xF0011A,'  RAM-only search.  !CCB ($F03EF0) and !DLY ($F02D9A) are each preceded by a'),
 (0xF0011A,'  STORE opcode, so those eye-catchers are written at runtime -- dynamic'),
 (0xF0011A,'  instances, simply never created in this configuration.  Same for !ASQ.'),
 (0xF0011A,'  LESSON FOR THE MARKER CENSUS: "no instance" meant "no instance IN RAM".'),
 (0xF0011A,'  Three of the twelve markers live in ROM or are created dynamically, and a'),
 (0xF0011A,'  census built from RAM dumps cannot tell "absent" from "elsewhere".'),
 # ---- !GST/!UST header named; +$0C is MENT not record size ----
 (0xF09EBE,'!GST/!UST HEADER FIELDS, per versados GST.EQ and UST.EQ:'),
 (0xF09EBE,'    +$00 eye-catcher   +$0C xxxMENT  MAXIMUM NUMBER OF ENTRIES'),
 (0xF09EBE,'    +$04 xxxNEXT       +$0E xxxCENT  current number of entries'),
 (0xF09EBE,'    +$08 xxxNSEG       +$10 xxxFENT  address of first entry'),
 (0xF09EBE,'    +$0A xxxNPAGE      +$14         first entry'),
 (0xF09EBE,'  CORRECTION: +$0C is MENT, MAX ENTRY COUNT -- not the record size.  The'),
 (0xF09EBE,'  capacity figure published here was CIRCULAR: $16 at +$0C was read as a'),
 (0xF09EBE,'  22-byte record size and then used as the divisor to compute "capacity'),
 (0xF09EBE,'  22" -- dividing by the very field being misnamed, and landing on the right'),
 (0xF09EBE,'  number because MENT IS 22.  Verified: (2*256 - $14) / $16 = 22 = USTMENT.'),
 (0xF09EBE,'  The record size is separately $16 because USTEL happens to be 22 in this'),
 (0xF09EBE,'  build -- two quantities that coincide, which is what hid the error.'),
 (0xF09EBE,'  UST ENTRY: +$00 USTTNAME, +$04 USTSESSN, +$08 USTSNAME "SEMAPHORE NAME",'),
 (0xF09EBE,'  +$0C USTUCNT (=1), +$0E USTXCNT, +$0F USTTYPE (=2 of 1/2/3), +$10 USTSEM.'),
 (0xF09EBE,'  USTSNAME holding AXP1/HXP1/AXP2 is a FOURTH confirmation of the semaphore'),
 (0xF09EBE,'  reading, after the CRSEM counts, the !UST expansion and the null TCBASQ.'),
 (0xF09EBE,'  REVISION TENSION: SR10 USTEL sums to 28 (it has USTSPTR + pad); this ROM'),
 (0xF09EBE,'  uses 22, omitting both -- an EARLIER UST.  Yet directive $4C = 76 is'),
 (0xF09EBE,'  BEYOND SR10\'s table, pointing LATER.  So this build is a different branch'),
 (0xF09EBE,'  or mixed sources, not simply "SR10 or newer".'),
 (0xF09E78,'INIT.SA (text/verdos10/M68XXX) IS THE SOURCE OF THIS SEQUENCE.  BLDGST is'),
 (0xF09E78,'  line-for-line what was decoded here: CLR the directory slot, load the size'),
 (0xF09E78,'  pc-relative, BEQ if zero, T0PAGAL, BSR KILLER on error, store the address,'),
 (0xF09E78,'  BSR TBLCLR, write the eye-catcher, then LSL #8 / SUB header / DIVU entry'),
 (0xF09E78,'  length to compute MENT.  The error bsr is KILLER, and STR.EQ gives'),
 (0xF09E78,'  T0KILLER EQU 32 "CRASH SYSTEM -- ERROR DETECTED": a failed allocation'),
 (0xF09E78,'  crashes the machine deliberately.'),
 # ---- !PAT named, and the $01 block is a Segment Parameter Block ----
 (0xF046B0,'THIS IS A SEGMENT PARAMETER BLOCK (SGPB), per versados SR10/U9995/SEG.EQ:'),
 (0xF046B0,'    +$00 SGPBTASK  target task name        = \'RDHC\''),
 (0xF046B0,'    +$04 SGPBSESS  session code            = 0'),
 (0xF046B0,'    +$08 SGPBOPT   directive options       = $2000'),
 (0xF046B0,'    +$0A SGPBATTR  segment attributes      = 0'),
 (0xF046B0,'    +$0C SGPBNAME  SEGMENT NAME            = \'STCK\''),
 (0xF046B0,'    +$10 SGPBLA    logical address         = 0'),
 (0xF046B0,'    +$14 SGPBSL    SEGMENT LENGTH IN BYTES = $190 = 400'),
 (0xF046B0,'  So \'STCK\' was NEVER A TAG -- it is SGPBNAME, a SEGMENT NAME, which is why'),
 (0xF046B0,'  it looks like the !xxx markers and is not one.  And SGPBOPT = $2000 is'),
 (0xF046B0,'  bit 13 = SGPBOPAD, "EXEC SUPPLIES LOGICAL ADDRESS (= PHYS ADDR)" -- exactly'),
 (0xF046B0,'  right for a machine with no address translation, and why SGPBLA is zero.'),
 (0xF046B0,'  THE WHOLE ALLOCATION CHAIN, in Motorola\'s own terms:'),
 (0xF046B0,'    GTSEG ($01) name=STCK len=$190 opt=SGPBOPAD'),
 (0xF046B0,'      -> T0PAGAL ($04) "ALLOCATE PHYSICAL PAGES", 400 rounded to 2x256'),
 (0xF046B0,'      -> a $200 segment, the per-task stride measured here'),
 (0xF046B0,'      -> address lands in TCBA6 (+$138); semaphore descriptors at its base,'),
 (0xF046B0,'         stack growing down from its top'),
 (0xF046B0,'  Every step was derived here from arithmetic and RAM dumps before any name'),
 (0xF046B0,'  was known, and the names confirm each one.'),
 (0xF09FA2,'!PAT = PERIODIC ACTIVATION TABLE, per SR10/U9995/PAT.EQ.  Its entry fields'),
 (0xF09FA2,'  sum to 4+4+4+4+4+2+4+2+2 = 30 = $1E, EXACTLY the free-list stride measured'),
 (0xF09FA2,'  here; PATFHDR at +$04 is exactly the "first-record pointer, a third header'),
 (0xF09FA2,'  shape" recorded; entries chain through PATNEXT at each entry +$00 as'),
 (0xF09FA2,'  measured; and the $FFFFFFFF at each entry +$04 is PATTCB -- "no task", the'),
 (0xF09FA2,'  free marker.  It is wholly free because the firmware never issues RQSTPA'),
 (0xF09FA2,'  (29) or T0RQPA (34).  An empty Periodic Activation Table in a system that'),
 (0xF09FA2,'  requests no periodic activations is not a mystery.'),
