...
DATA_RANGES = [
    # F0A57E..F0B17E was previously locked as DATA, but the first ~80 bytes
    # are reachable code (panel-command-arg setter entered via a computed
    # dispatch). Keep the range narrowed so the recursive descent can claim
    # the prologue while the !TCB tables that follow still won't be walked
    # (no Bcc/JSR points into them). The trailing zero-fill is locked.
    (0xF0B17E, 0xF10000, "Zero-fill / ROM checksum word at F0FFFE"),
]
...
# ----------------------------------------------------------------------
# JMP-PC dispatch-table scan
# ----------------------------------------------------------------------
# State-machine dispatchers in this ROM use a uniform pattern: a 20-entry
# table of `JMP d16(PC)` instructions (opcode 4E FA, 4 bytes each), often
# preceded by a `NOP` (4E 71) for alignment. Recursive descent never enters
# the middle of such a table because the index is computed at runtime
# (`lea TBL,a4` then `JMP (a4,dN.w)`-style indexed branch).
#
# Heuristic: find runs of ≥4 consecutive `4E FA XX YY` words at even
# offsets, take each as a code seed. Validate that the target is in-range
# and even. Optionally tolerate a leading NOP word.
def scan_jmp_pc_tables(rom, min_run=4):
    """Seed both the table ENTRIES (so `4E FA XX YY` decodes as JMP, not
    DC.W) and their dispatch TARGETS (so the dispatched code is walked)."""
    seeds = set()
    n = len(rom)
    off = 0
    while off + 4 <= n:
        start = off
        had_nop = (rom[off] == 0x4E and rom[off+1] == 0x71)
        if had_nop:
            start = off + 2
        run = 0
        cur = start
        while cur + 4 <= n and rom[cur] == 0x4E and rom[cur+1] == 0xFA:
            d16 = struct.unpack(">h", rom[cur+2:cur+4])[0]
            tgt = (BASE + cur + 2 + d16) & 0x00FFFFFF
        ...
        if run >= min_run:
            if had_nop:
                seeds.add(BASE + off)         # NOP alignment word
            cur = start
            for _ in range(run):
                seeds.add(BASE + cur)         # the JMP entry itself
                d16 = struct.unpack(">h", rom[cur+2:cur+4])[0]
                tgt = (BASE + cur + 2 + d16) & 0x00FFFFFF
                seeds.add(tgt)                # the dispatch target
                cur += 4
            # Continue past the JMP run: trailing slots in these 20-entry
            # dispatch tables hold "do-nothing" handlers — RTS+NOP, RTS+RTS,
            # or another JMP. Accept any 4-byte slot whose first byte is 0x4E
            # (the m68k MISC group: JMP/JSR/RTS/NOP/RTE/RTR/RTD/TRAP) until
            # we hit something else. Cap at 20 total slots (typical layout).
            slots = run
            while slots < 20 and cur + 4 <= n and rom[cur] == 0x4E:
                seeds.add(BASE + cur)
                if rom[cur+1] == 0xFA:                     # JMP d16(PC)
                    d16 = struct.unpack(">h", rom[cur+2:cur+4])[0]
                    tgt = (BASE + cur + 2 + d16) & 0x00FFFFFF
...
def main():
    ...
    tier_c = scan_jmp_pc_tables(rom) - tier_a - tier_b
    seeds = tier_a | tier_b | tier_c
    print(f"jmp-pc-table seeds: {len(tier_c)}")
    ...
    with open(OUT_PATH, "w") as f:
        ...
        # Pre-scan for "certified" ASCII string runs in non-code regions:
        # ≥4 consecutive printable ASCII bytes (0x20–0x7E) that aren't
        # claimed by `code`. Random binary data effectively never produces
        # 4+ printable bytes in a row, so this is a high-precision rule
        # (false-positive rate measured at < 0.1% on this ROM).
        string_mask = bytearray(END - START)
        i = 0
        n = END - START
        while i < n:
            ai = START + i
            if ai in code:
                i += code[ai][0]; continue
            run = 0
            while i + run < n:
                ar = START + i + run
                if ar in code: break
                b = rom[ar - BASE]
                if not (0x20 <= b <= 0x7E): break
                run += 1
            if run >= 4:
                # Reject runs that are actually m68k single-word
                # instructions misread as text. RTS=4E75, NOP=4E71,
                # RTE=4E73, RTR=4E77, TRAP=4Exx — these all start with
                # 0x4E. If >25% of the run's bytes are 0x4E, it's
                # almost certainly instruction filler from a dispatch-
                # table tail, not a string.
                bs = rom[START - BASE + i : START - BASE + i + run]
                ne_count = bs.count(0x4E)
                if ne_count * 4 < run:   # < 25% 'N' bytes → real text
                    for k in range(run):
                        string_mask[i + k] = 1
            i += max(run, 1)
        ...
        while addr < END:
            ...
            if string_mask[addr - START] and addr not in code:
                # Emit certified ASCII string as DC.B "..."
                run_end = addr
                while run_end < END and string_mask[run_end - START] \
                        and run_end not in code:
                    run_end += 1
                s = rom[addr - BASE : run_end - BASE].decode('ascii')
                # escape backslash and double-quote
                esc = s.replace('\\', '\\\\').replace('"', '\\"')
                bytestr = " ".join(f"{b:02x}" for b in rom[addr-BASE:run_end-BASE][:6])
                if run_end - addr > 6: bytestr += " .."
                f.write(f'{addr:06X}  {bytestr:<22}  DC.B     "{esc}"  ; {run_end-addr} bytes\n')
                addr = run_end
                continue
            ...
            if addr in code:
                ...
                if data_mask[addr - START]:
                    # Known data table. Prefer DC.L when at 4-byte alignment
                    # and the value resolves to a labelled in-range address —
                    # this turns scattered DC.W pairs into readable pointer
                    # tables (TCB entry-point fields, dispatch tables, etc.).
                    if (addr & 3) == 0 and off + 4 <= len(rom):
                        v = struct.unpack(">I", rom[off:off+4])[0] & 0x00FFFFFF
                        if v in referenced and v != 0:
                            bs = " ".join(f"{b:02x}" for b in rom[off:off+4])
                            f.write(f"{addr:06X}  {bs:<22}  DC.L     {labelfor(v)}\n")
                            addr += 4
                            continue
                    if off + 2 <= len(rom):
                    ...
                    # Same DC.L-when-pointer heuristic as the data_mask branch
                    if (addr & 3) == 0 and off + 4 <= len(rom):
                        v = struct.unpack(">I", rom[off:off+4])[0] & 0x00FFFFFF
                        if v in referenced and v != 0:
                            bs = " ".join(f"{b:02x}" for b in rom[off:off+4])
